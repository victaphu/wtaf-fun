# Hackathon Project changes for EthGlobal Agentic Hackathon 2025

## BattleRoyale AI Game Master

BattleRoyale is an innovative on-chain game where AI acts as a game master, controlling elimination rounds for meme tokens. Here's how it works:

### Game Mechanics
- Players create and stake meme tokens to participate
- AI Game Master runs 5 elimination rounds
- Each round eliminates 50% of participants based on dynamic rules
- Winners split the staked rewards pool

### Smart Contract Architecture
- `BattleRoyale.sol`: Main game controller
  - Handles token staking
  - Manages game rounds
  - Controls eliminations
  - Distributes rewards

### AI Game Master Features
- Fetches active token list from contract
- Caches participant data
- Generates creative elimination rules
- Executes 5 rounds of elimination
- Determines final winners
- Triggers new game initialization

### Technical Implementation

// Example AI Game Master implementation
const battleRoyaleAI = await AgentKit.from({
  cdpApiKeyName: "BATTLE_ROYALE_AI",
  cdpApiKeyPrivate: "YOUR_PRIVATE_KEY",
  contractAddress: "BATTLE_ROYALE_CONTRACT_ADDRESS",
  
  // Game configuration
  roundCount: 5,
  eliminationRate: 0.5,
  minStakeAmount: "1000000000000000", // 0.0001 ETH
  
  // Custom actions
  actions: {
    fetchActivePlayers: async () => {/* ... */},
    generateEliminationRules: async () => {/* ... */},
    executeRound: async (round: number) => {/* ... */},
    eliminatePlayers: async (players: string[]) => {/* ... */},
